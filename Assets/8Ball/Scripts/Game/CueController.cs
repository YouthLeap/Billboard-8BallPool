...
public class CueController : MonoBehaviour
{
    ...
    public Transform potDownRight_AI;
    public Transform potDownCenter_AI;
    public Transform potDownLeft_AI;
    public Transform potUpRight_AI;
    public Transform potUpCenter_AI;
    public Transform potUpLeft_AI;

    void Start()
	{
        GameManager.Instance.Ball8ForSolid = false;
		GameManager.Instance.Ball8ForStriped = false;
		...
		{
            if (!GameManager.Instance.Player_AI && (GameManager.Instance.Player_AI && GameManager.Instance.offlinePlayerTurn == 1))
	...
	private void setPrizeText()
	{
		...
		if(GameManager.Instance.offlineMode) {
            if (GameManager.Instance.Player_AI)
                prizeText.GetComponent<Text>().text = (GameManager.Instance.payoutCoins * 2).ToString();
            else
	...
	private void OnEvent(byte eventcode, object content, int senderid)
	{
		...
		{
			if(canShowControllers && isServer && !shotMyTurnDone || (GameManager.Instance.Player_AI && GameManager.Instance.offlinePlayerTurn != 2))
	...
	public void setTurnOffline(bool showTurnMessage) {
        ...
        if (GameManager.Instance.Player_AI)
        {
            potDownRight_AI.Find("CalledPocket_AI").gameObject.SetActive(false);
            potDownCenter_AI.Find("CalledPocket_AI").gameObject.SetActive(false);
            potDownLeft_AI.Find("CalledPocket_AI").gameObject.SetActive(false);
            potUpRight_AI.Find("CalledPocket_AI").gameObject.SetActive(false);
            potUpCenter_AI.Find("CalledPocket_AI").gameObject.SetActive(false);
            potUpLeft_AI.Find("CalledPocket_AI").gameObject.SetActive(false);
        ...
        if (GameManager.Instance.Player_AI && GameManager.Instance.offlinePlayerTurn == 1)
        {
            if (GameManager.Instance.tableNumber > 3)
            {
                GameManager.Instance.callPocketBlack = true;
        ...
        //}
        //else if (GameManager.Instance.Player_AI && GameManager.Instance.offlinePlayerTurn == 2)
        //{
        ...
        StartCoroutine("StartRotateCue");
       // StartCoroutine("GetPot_AI");
		...
		if(!GameManager.Instance.iWon && !GameManager.Instance.iLost ) {
            ...
            if(!GameManager.Instance.Player_AI || (GameManager.Instance.Player_AI && GameManager.Instance.offlinePlayerTurn != 2))
			    ShotPowerIndicator.anim.Play("MakeVisible");
		}
	}

    private void shot_AI()
    {
        if (GameManager.Instance.Player_AI && GameManager.Instance.offlinePlayerTurn == 2)
        {
            steps = (int)(UnityEngine.Random.Range(15.1f, 35.1f));
            if (steps > 0)
            {
                shouldShot = true;
                startShoot = true;
    ...
    public IEnumerator StartRotateCue()
    {        
        while (true)
        {
            if (GameManager.Instance.Player_AI && GameManager.Instance.offlinePlayerTurn == 2)
            {
                x++;
                cue.transform.rotation = Quaternion.Euler(0, 0, x);
                //drawTargetLines();

                if (GameManager.Instance.ownSolids)
                {
                    if (ballNumber == 1 || ballNumber == 2 || ballNumber == 3 || ballNumber == 4 || ballNumber == 5 || ballNumber == 6 || ballNumber == 7)
                    {
                        StartCoroutine("GetPot_AI");
                        break;
                    }

                    if (GameManager.Instance.Ball8ForSolid && ballNumber == 8)
                    {
                        StartCoroutine("GetPot_AI");
                        break;
                    }
                ...
                else if (!GameManager.Instance.ownSolids)
                {
                    if (ballNumber == 9 || ballNumber == 10 || ballNumber == 11 || ballNumber == 12 || ballNumber == 13 || ballNumber == 14 || ballNumber == 15 )
                    {
                        StartCoroutine("GetPot_AI");
                        break;
                    }

                    if (GameManager.Instance.Ball8ForStriped && ballNumber == 8)
                    {
                        StartCoroutine("GetPot_AI");
                        break;
                    }                   
    ...
    public IEnumerator GetPot_AI()
    {        
        while (true)
        {
            if (GameManager.Instance.Player_AI && GameManager.Instance.offlinePlayerTurn == 2 )
            {
                GameManager.Instance.rayOrigin_AI.z = 0;
                GameManager.Instance.rayDirection_AI = GameManager.Instance.rayDirection_AI.normalized;
                RaycastHit hit1;

                if (GameManager.Instance.ownSolids)
                {
                    if (ballNumber == 1 || ballNumber == 2 || ballNumber == 3 || ballNumber == 4 ||
                    ballNumber == 5 || ballNumber == 6 || ballNumber == 7 || (GameManager.Instance.Ball8ForSolid && ballNumber == 8))
                    {
                        x += 0.05f;
                    }
                    else
                ...
                {
                    if (ballNumber == 9 || ballNumber == 10 || ballNumber == 11 || ballNumber == 12 ||
                        ballNumber == 13 || ballNumber == 14 || ballNumber == 15 || (GameManager.Instance.Ball8ForSolid && ballNumber == 8))
                    {
                        x += 0.05f;
                    }
                    else
                ...
                if (Physics.Raycast(GameManager.Instance.rayOrigin_AI, GameManager.Instance.rayDirection_AI, out hit1, 50))
                {
                    Debug.DrawRay(GameManager.Instance.rayOrigin_AI, GameManager.Instance.rayDirection_AI * 50, Color.yellow);
                    if (hit1.transform.tag.Contains("Pot_AI") )
                    {
                        if (hit1.transform.tag.Contains("Pot_AI1") && GameManager.Instance.tableNumber > 3)
                            potUpLeft_AI.Find("CalledPocket_AI").gameObject.SetActive(true);
                        else if(hit1.transform.tag.Contains("Pot_AI2") && GameManager.Instance.tableNumber > 3)
                            potUpCenter_AI.Find("CalledPocket_AI").gameObject.SetActive(true);
                        else if(hit1.transform.tag.Contains("Pot_AI3") && GameManager.Instance.tableNumber > 3)
                            potUpRight_AI.Find("CalledPocket_AI").gameObject.SetActive(true);
                        else if(hit1.transform.tag.Contains("Pot_AI4") && GameManager.Instance.tableNumber > 3)
                            potDownLeft_AI.Find("CalledPocket_AI").gameObject.SetActive(true);
                        else if(hit1.transform.tag.Contains("Pot_AI5") && GameManager.Instance.tableNumber > 3)
                            potDownCenter_AI.Find("CalledPocket_AI").gameObject.SetActive(true);
                        else if(hit1.transform.tag.Contains("Pot_AI6") && GameManager.Instance.tableNumber > 3)
                            potDownRight_AI.Find("CalledPocket_AI").gameObject.SetActive(true); 

                        GameManager.Instance.rayOrigin_AI.z = -5;
                        Invoke("shot_AI", 2.0f);
                        break;
                    }
                }
    ...
    private void drawTargetLines()
	{
		...
		if (!ballsInMovement)
		{
			...
			if (Physics.SphereCast(ray, radius, out hitInfo, 10, layerMask))
			{
				...
				{
                    ballNumber = System.Int32.Parse(hitInfo.transform.tag.Replace("Ball", ""));
					Debug.Log ("finding ball number "+ballNumber);
					if (GameManager.Instance.playersHaveTypes) {
						Debug.Log ("Finding ball : " + ballNumber);
						if (isServer) {
							if ((GameManager.Instance.ownSolids && ballNumber < 8) || (!GameManager.Instance.ownSolids && ballNumber > 8) /*|| ballNumber == 8*/) {
								drawLine (linePos);
							} else {
								if (GameManager.Instance.ownSolids && GameManager.Instance.Ball8ForSolid && ballNumber == 8) {
									drawLine (linePos);
								} else if (!GameManager.Instance.ownSolids && GameManager.Instance.Ball8ForStriped && ballNumber == 8) {
									drawLine (linePos);
								} else {
						...
						if ((GameManager.Instance.stripedPoted == 0 || GameManager.Instance.solidPoted == 0) && ballNumber == 8) {
	...
	public void drawLine(Vector3 linePos)
	{
        ...
        GameManager.Instance.rayOrigin_AI = hitBallPosition;
        GameManager.Instance.rayDirection_AI = r2dir;       
