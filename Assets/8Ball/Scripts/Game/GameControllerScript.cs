	...
	void Start() {
		...
		if (GameManager.Instance.offlineMode) {
            ...
            if (GameManager.Instance.Player_AI)
            {
                ...
                GameObject.Find("Name2").GetComponent<Text>().text = GameManager.Instance.opponentName_AI;
            }
            else {
	...
	private void updateClock() {
		...
		if (currentImage == 1) {
			...
			if (imageClock1.fillAmount == 0) {
				GameManager.Instance.ballHand.SetActive(true);
				GameManager.Instance.hasCueInHand = true;

				Vector3 newBallPos = GameManager.Instance.whiteBall.transform.position;
				newBallPos.z = GameManager.Instance.ballHand.transform.position.z;
				GameManager.Instance.ballHand.transform.position = newBallPos;
