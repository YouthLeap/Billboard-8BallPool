...
public class LockZPosition : MonoBehaviour {
	...
	void Start() {
		GameManager.Instance.firstBallPot = false;
	...
	void OnTriggerEnter(Collider other) {
		...
		if (!poted && other.tag.Contains("Pot")) {
			...
			if (!GameManager.Instance.firstBallPot) {
				if (transform.tag.Equals("WhiteBall")) {
					...
					if (ballNumber == 8) {
						...
						GameManager.Instance.firstBallPot = true;
			...
			if (transform.tag.Equals("WhiteBall")) {
				...
				if (ballNumber < 8) {
					...
					if(GameManager.Instance.solidPoted==7){
						GameManager.Instance.Ball8ForSolid = true;
					}
				} else if (ballNumber > 8) {
					GameManager.Instance.stripedPoted++;
					if(GameManager.Instance.stripedPoted==7){
						GameManager.Instance.Ball8ForStriped = true;
					}
				}
