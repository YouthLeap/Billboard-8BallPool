...
public class WinnerControllerScript : MonoBehaviour {
    ...
    void OnEnable() {
        ...
        if (!isGameScene) {
           ...
           // if (StaticStrings.showAdOnGameOverScene)
            ...
            if (GameManager.Instance.iWon) {    
                    ...
                    audioSources[0].Play();
                    int tempWinningAmmount = (int)((GameManager.Instance.payoutCoins * 2) * GameManager.Instance.CommissionPercentage) + 1;
                    Debug.Log("===========winner============ : " + tempWinningAmmount);
                    GameManager.Instance.playfabManager.addCoinsRequest(tempWinningAmmount);                
            } else {                
            ...
            if(GameManager.Instance.Player_AI)
                oppoText.text = GameManager.Instance.opponentName_AI;
            else
			...
			int prizeCoins = (int)((GameManager.Instance.payoutCoins * 2) * GameManager.Instance.CommissionPercentage) + 1;
			//AssemblyLoadEventArgs;dkjfalslkdjfaksdjfsdd
        ...
        if(GameManager.Instance.iWon && GameManager.Instance.Player_AI)
        {
            int val = (int)((GameManager.Instance.CommissionPercentage * (GameManager.Instance.payoutCoins * 2)) + 1);
            GameManager.Instance.playfabManager.updateScoreOnOxBoxServer(val.ToString());
            GameManager.Instance.coinsCount += val;
        }
    }
    ...
    public void loadMenuScene() {
        ...
        GameManager.Instance.otherPlayerJoined = false;
        GameManager.Instance.Player_AI = false;
