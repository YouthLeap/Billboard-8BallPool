...
public class FacebookManager : MonoBehaviour {
    ...
    public GameObject registerValidation;
    public Text registerValidationText;
    ...
    private void InitCallback() {
        //Debug.LogError("FB.IsInitialized " + FB.IsInitialized);  //fb
        //if (FB.IsInitialized) {
    ...
    public void FBLogin() {
        if (!LoggedIn) {
          ...
          //  FB.LogInWithReadPermissions(perms, AuthCallback); //fb
        } else {
            playFabManager.JoinRoomAndStartGame();
        }
    ...
    //private void AuthCallback(ILoginResult result) { //fb
    //    if (FB.IsLoggedIn) {
    ...
    private void initSession() {
        ...
        if (logType.Equals("Facebook")) {
            //GameManager.Instance.facebookIDMy = Facebook.Unity.AccessToken.CurrentAccessToken.UserId; //fb
            callApiToGetName();
            getMyProfilePicture(GameManager.Instance.facebookIDMy);
    ...
    private void callApiToGetName() {
        //FB.API("me?fields=first_name", Facebook.Unity.HttpMethod.GET, APICallbackName); //fb
    }


    //void APICallbackName(IResult response) {  //fb
