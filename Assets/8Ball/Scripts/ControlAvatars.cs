...
public class ControlAvatars : MonoBehaviour {
    ...
    private string[] player_ai_name = new string[] { "Thor", "Spider", "Batman", "Isop", "Lonny", "Captin",
                                                    "Boss", "Jack", "Jane", "Eddie", "Jimmy", "Matty", "Bert", "Alex"};
    ...
    public void test() {
        ...
        {
            PhotonNetwork.LeaveRoom();           
            GameManager.Instance.Player_AI = true;
            GameManager.Instance.offlineMode = true;

            Index = (int)(Random.Range(0.1f, 13.1f));

            opponentNameText.text = player_ai_name[Index];
            GameManager.Instance.opponentName_AI = player_ai_name[Index];

            Add_AI_Avatar();

            Invoke("AI_Avatar_Ani", 5.0f);
            Invoke("StartGameScene_AI", 11.0f);
        }        
    }

    private void AI_Avatar_Ani()
    {
        coinsAnimate();
        AvatarFrameMy.GetComponent<Animator>().Play("MySelectorMoveOut");
        AvatarFrameOpponent.GetComponent<Animator>().Play("OpponentFrameMoveOut");
    ...
    private void StartGameScene_AI()
    {
        GameManager.Instance.playfabManager.UpdatedFreeCreditAd((GameManager.Instance.coinsCount - GameManager.Instance.payoutCoinsWithoutComission).ToString());
        GameManager.Instance.coinsCount -= GameManager.Instance.payoutCoinsWithoutComission;
